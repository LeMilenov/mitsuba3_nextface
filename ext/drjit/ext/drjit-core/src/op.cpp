...
void jitc_var_new_reservoir_sampling(ReservoirSampleData& reservoir_data, uint32_t& lock,
                                     const ReservoirSampleData& sample_data,
                                     uint32_t random_num, uint32_t index, uint32_t mask) {
    if (random_num == 0 && index == 0 && mask == 0 && !reservoir_sample_data_initialized(sample_data))
        return;
    if (unlikely(!reservoir_sample_data_initialized(reservoir_data) || lock == 0 ||
                 !reservoir_sample_data_initialized(sample_data) ||
                 random_num == 0 || index == 0 || mask == 0)) {
        jitc_raise("jit_var_new_reservoir_sampling(): uninitialized arguments!");
    }

    auto print_log = [&](const char *reason) {
        jitc_log(Debug, "jit_var_new_reservoir_sampling(%s)", reason);
    };
    ...
    uint32_t input_arrays[] = { sample_data.sample_idx,
        sample_data.u, sample_data.v, sample_data.w,
        sample_data.value_r, sample_data.value_g, sample_data.value_b,
        random_num, index, mask, sample_data.weight };

    // Get size, ensure no arrays are dirty
    for (uint32_t idx : input_arrays) {
        const Variable *v = jitc_var(idx);
        size = std::max(v->size, size);
        dirty |= (bool) v->is_dirty();
        // Fetch backend from 'weight'
        backend = (JitBackend) v->backend;
    }

    if (backend != JitBackend::CUDA) {
        jitc_raise("jit_var_new_reservoir_sampling(): only the CUDA backend is supported!");
    }
    ...
    if (dirty) {
        ...
        for (uint32_t idx : input_arrays) {
            if (jitc_var(idx)->is_dirty())
                jitc_fail("jit_var_new_reservoir_sampling(): variable remains dirty after "
                          "evaluation!");
        }
    }

    if (ts->compute_capability < 70) {
        jitc_fail("jit_var_new_reservoir_sampling(): Compute capability at least 7.0 required!");
    }
    ...
    for (uint32_t idx : input_arrays) {
        ...
        if (size != v->size && v->size != 1)
            jitc_raise("jit_var_new_reservoir_sampling(): arrays of incompatible size!");
    }

    // Don't do anything if the mask is always false
    {
        const Variable *v_mask = jitc_var(mask);
        if (v_mask->literal && v_mask->literal == 0) {
            print_log("skipped, always masked");
            return;
        }
    }

    uint32_t inputs[] = { sample_data.weight, sample_data.sample_idx,
        sample_data.u, sample_data.v, sample_data.w,
        sample_data.value_r, sample_data.value_g, sample_data.value_b };
    Ref targets[] = {
        borrow(lock),
        borrow(reservoir_data.weight),
        borrow(reservoir_data.sample_idx),
        borrow(reservoir_data.u),
        borrow(reservoir_data.v),
        borrow(reservoir_data.w),
        borrow(reservoir_data.value_r),
        borrow(reservoir_data.value_g),
        borrow(reservoir_data.value_b),
    };
    constexpr size_t num_inputs = sizeof(inputs) / sizeof(uint32_t);
    constexpr size_t num_targets = sizeof(targets) / sizeof(Ref);

    {
        Variable *v_targets[num_targets];
        for (uint32_t i = 0; i < num_targets; i++) {
            v_targets[i] = jitc_var(targets[i]);
        }
        const Variable *v_weight  = jitc_var(sample_data.weight);

        for (const Variable *v_target : v_targets) {
            if (v_target->placeholder)
                jitc_raise("jit_var_new_reservoir_sampling(): cannot write to a placeholder variable!");
        }

        if (v_weight->literal && v_weight->literal == 0) {
            print_log("skipped, jit_var_new_reservoir_sampling() with zero-valued weights");
            lock = targets[0].release();
            reservoir_data.weight = targets[1].release();
            reservoir_data.sample_idx = targets[2].release();
            reservoir_data.u = targets[3].release();
            reservoir_data.v = targets[4].release();
            reservoir_data.w = targets[5].release();
            reservoir_data.value_r = targets[6].release();
            reservoir_data.value_g = targets[7].release();
            reservoir_data.value_b = targets[8].release();
            return;
        }

        // Check if it is safe to write directly
        for (uint32_t i = 0; i < num_targets; i++) {
            const Variable *v_target = v_targets[i];
            if (v_target->ref_count > 2)
                targets[i] = steal(jitc_var_copy(targets[i]));
        }
    }

    Ref ptrs[num_targets];
    for (uint32_t i = 0; i < num_targets; i++) {
        ptrs[i] = steal(jitc_var_pointer(backend, jitc_var_ptr(targets[i]), targets[i], 1));
    }
    ...
    bool unmasked = false;
    {
        ...
        unmasked   = v_mask->literal  && v_mask->literal  == 1;
        size = std::max(size, v_mask->size);
    }

    uint32_t dep[3] = { random_num, index, mask };
    uint32_t n_dep = 3;
    ...
    uint32_t result =
        jitc_var_stmt(backend, VarType::Void, "", 0, n_dep, dep);

    jitc_var(result)->extra = 1;
    Extra &extra = state.extra[result];
    extra.n_dep = num_inputs + num_targets;
    extra.dep = (uint32_t *) malloc(sizeof(uint32_t) * extra.n_dep);

    for (uint32_t i = 0; i < num_targets; i++) {
        extra.dep[i] = ptrs[i];
        jitc_var_inc_ref(ptrs[i]);
    }
    for (uint32_t i = 0; i < num_inputs; i++) {
        extra.dep[num_targets + i] = inputs[i];
        jitc_var_inc_ref(inputs[i]);
    }

    extra.assemble = [](const Variable *v, const Extra &extra) {
        const Variable *v_rand_num  = jitc_var(v->dep[0]);
        const Variable *v_idx       = jitc_var(v->dep[1]);

        uint32_t v_i        = v->reg_index;

        bool masked = false; 
        const Variable *v_mask = nullptr;
        if (v->dep[2]) {
            v_mask = jitc_var(v->dep[2]);
        ...
        buffer.fmt("    {\n"
                   ...
                   "         );\n"
                   "    }\n",
            v_idx->reg_index, v_rand_num->reg_index,
            jitc_var(extra.dep[0])->reg_index, jitc_var(extra.dep[1])->reg_index,
            jitc_var(extra.dep[2])->reg_index, jitc_var(extra.dep[3])->reg_index,
            jitc_var(extra.dep[4])->reg_index, jitc_var(extra.dep[5])->reg_index,
            jitc_var(extra.dep[6])->reg_index, jitc_var(extra.dep[7])->reg_index,
            jitc_var(extra.dep[8])->reg_index, jitc_var(extra.dep[9])->reg_index,
            jitc_var(extra.dep[10])->reg_index, jitc_var(extra.dep[11])->reg_index,
            jitc_var(extra.dep[12])->reg_index, jitc_var(extra.dep[13])->reg_index,
            jitc_var(extra.dep[14])->reg_index, jitc_var(extra.dep[15])->reg_index,
            jitc_var(extra.dep[16])->reg_index
        );
        if (masked)
