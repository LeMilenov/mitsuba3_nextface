...
void jit_var_new_reservoir_sampling(ReservoirSampleData& reservoir_data, uint32_t& lock,
                                    const ReservoirSampleData& sample_data,
                                    uint32_t random_num, uint32_t index, uint32_t mask) {
    lock_guard guard(state.lock);
    jitc_var_new_reservoir_sampling(reservoir_data, lock, sample_data,
                                    random_num, index, mask);
}
