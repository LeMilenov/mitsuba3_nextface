...
static inline bool reservoir_sample_data_initialized(const ReservoirSampleData& r) {
    return r.weight != 0 && r.sample_idx != 0 &&
        r.u != 0 && r.v != 0 && r.w != 0 &&
        r.value_r != 0 && r.value_g != 0 && r.value_b != 0;
}

/**
 * \brief Atomically resample and update weights used for weighted reservoir sampling
 */
extern JIT_EXPORT void jit_var_new_reservoir_sampling(ReservoirSampleData& reservoir_data, uint32_t& lock,
